...
export default function DataTable() {
  ...
  const [buttons, setButtons] = useState([
    <Button label="" className="bg-gray-100" onClick={() => setNumber(3)} />,
  ]);
  ...
  const columnClass = cx({
    "px-4 py-2.5 text-gray-500 text-left text-sm": true,
  });

  const columnContentClass = cx({
    "px-4 py-2 text-left bg-gray-50": true,
  });
      ...
      <table className="table table-striped w-full">
        <thead className="bg-gray-100">
          <tr>
            ...
            <th className={columnClass}> Duration (min)</th>
      ...
      <div className="flex flex-row w-full justify-between px-6 bg-gray-50 items-center">
        <div className="text-s">
          {number * page <= movies.length
            ...
            : `Prikazuje se ${page} do ${movies.length} od ${movies.length} unosa`}
        </div>
